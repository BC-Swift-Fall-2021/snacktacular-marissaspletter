...
//  Snacktacular
//
//  Created by Marissa Spletter on 11/11/21.
//
...
class Photo {
    ...
    func saveData(spot: Spot, completion: @escaping (Bool) -> ()) {
        ...
        uploadTask.observe(.failure) { (snapshot) in
            if let error = snapshot.error {
                print("ERROR: uplaod task for file \(self.documentID) failed, in spot \(spot.documentID), with error \(error.localizedDescription)")
            }
            completion(false)
        }
    }
    ...
    private func deleteImage(spot: Spot) {
        ...
        storageRef.delete { error in
            if let error = error {
                print("Error: could not delte photo \(error.localizedDescription)")
            } else {
                print("The photo was successfully printed")
            }
        }
    }
}
